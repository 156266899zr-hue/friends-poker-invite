...
test('好友权限、邀请、反馈持久化',async t=>{
 ...
 const item=(await f.request('/admin-api',{op:'feedbackList'},admin)).data.items[0];assert.equal(item.userId,friend.id);assert.equal(item.roomId,room.code);assert.equal(item.gameVersion,'1.3.0');
