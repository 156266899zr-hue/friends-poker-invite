...
export function view(r,id,historyAfter=0){const p=r.players.find(p=>p.id===id),now=Date.now(),playing=!['waiting','done'].includes(r.phase),allHistory=r.history||[],cursor=Number.isSafeInteger(historyAfter)&&historyAfter>0&&historyAfter<=(r.eventSequence||0)?historyAfter:0,history=cursor?allHistory.filter(e=>e.sequence>cursor):allHistory;return {code:r.code,host:r.host,phase:r.phase,hand:r.hand,board:r.board,dealer:r.dealer,turn:r.turn,deadline:r.deadline,expiresAt:r.expiresAt||0,high:r.high,pot:r.players.reduce((s,p)=>s+p.total,0),result:r.result,settlement:r.phase==='done'?r.settlement||[]:[],lastSettlement:r.lastSettlement||null,history,historyDelta:!!cursor,historyStartHand:allHistory[0]?.hand||r.hand,me:id,legal:p?legal(r,p):null,players:r.players.map(p=>{const level=levelForXp(p.xp||0),tier=tierForLevel(level);return {id:p.id,name:p.name,bot:p.bot,cardBack:cleanCardBack(p.cardBack),xp:p.xp||0,level,levelTier:tier.key,levelTierName:tier.name,online:p.bot||now-p.lastSeen<5000,pendingKick:!!p.pendingKick,rebirthCount:p.rebirthCount||0,rebirthAt:p.rebirthAt||0,canRebirth:!p.bot&&p.stack<progressionConfig.rebirthMinimumStack&&['waiting','done'].includes(r.phase),record:p.stack-(p.issued||2000),spectating:playing&&!p.inHand,stack:p.stack,bet:p.bet,total:p.total,inHand:p.inHand,folded:p.folded,action:p.action,cards:p.id===id||(r.phase==='done'&&r.reveal&&active(p))?p.cards:p.cards.map(()=>-1)};} )};}
