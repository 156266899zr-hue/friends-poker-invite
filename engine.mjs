...
export function player(name,bot=false,deviceId='',cardBack='classic'){return {id:randomUUID(),name,bot,deviceId,cardBack:cleanCardBack(cardBack),stack:2000,issued:2000,refillCount:0,refillPending:false,cards:[],bet:0,total:0,folded:false,inHand:false,lastSeen:Date.now(),pendingKick:false};}
...
export function start(r){
 if(!['waiting','done'].includes(r.phase))throw Error('本手牌尚未结束');if(r.players.filter(p=>p.stack>0).length<2)throw Error('至少需要两位有积分的玩家');
 ...
 for(const p of r.players){p.inHand=p.stack>0;p.folded=false;p.bet=0;p.total=0;p.lastHigh=null;p.cards=p.inHand?[r.deck.pop(),r.deck.pop()]:[];p.action=p.inHand?'等待行动':'积分不足';}
...
export function requestRefill(r,id){if(!['waiting','done'].includes(r.phase))throw Error('请在本手结束后申请补分');const p=r.players.find(p=>p.id===id&&!p.bot);if(!p)throw Error('玩家不存在');if(p.stack!==0)throw Error('积分归零后才能申请补分');if(p.refillPending)throw Error('补分申请已提交');p.refillPending=true;p.action='等待补分审批';log(r,`${p.name} 申请补充积分`);return p;}
export function respondRefill(r,id,approved){if(!['waiting','done'].includes(r.phase))throw Error('请在本手结束后处理补分');const p=r.players.find(p=>p.id===id&&!p.bot);if(!p||!p.refillPending)throw Error('补分申请不存在或已处理');p.refillPending=false;if(!approved){p.action='补分申请未通过';log(r,`${p.name} 的补分申请未通过`);return p;}if(p.stack!==0)throw Error('该玩家当前无需补分');p.stack=2000;p.issued=(p.issued||2000)+2000;p.refillCount=(p.refillCount||0)+1;p.action='已补分，等待开始';log(r,`${p.name} 经房主批准补充 2,000 积分 · 累计战绩 ${(p.stack-p.issued).toLocaleString('zh-CN')}`);return p;}
export function view(r,id){const p=r.players.find(p=>p.id===id),now=Date.now(),playing=!['waiting','done'].includes(r.phase);return {code:r.code,host:r.host,phase:r.phase,hand:r.hand,board:r.board,dealer:r.dealer,turn:r.turn,deadline:r.deadline,expiresAt:r.expiresAt||0,high:r.high,pot:r.players.reduce((s,p)=>s+p.total,0),result:r.result,settlement:r.phase==='done'?r.settlement||[]:[],lastSettlement:r.lastSettlement||null,history:r.history||[],me:id,legal:p?legal(r,p):null,players:r.players.map(p=>({id:p.id,name:p.name,bot:p.bot,cardBack:cleanCardBack(p.cardBack),online:p.bot||now-p.lastSeen<5000,pendingKick:!!p.pendingKick,refillPending:!!p.refillPending,refillCount:p.refillCount||0,record:p.stack-(p.issued||2000),spectating:playing&&!p.inHand,stack:p.stack,bet:p.bet,total:p.total,inHand:p.inHand,folded:p.folded,action:p.action,cards:p.id===id||(r.phase==='done'&&r.reveal&&active(p))?p.cards:p.cards.map(()=>-1)}))};}
