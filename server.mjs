...
const rooms=new Map(),port=Number(process.env.PORT||8787);const root=new URL('./',import.meta.url);
...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(b.op==='create'){if(rooms.size>=100)throw Error('房间已满，请稍后再试');let code;do{code=String(randomInt(100000,1000000));}while(rooms.has(code));const p=player(String(b.name||'朋友').trim().slice(0,12)||'朋友');r=room(code,p);rooms.set(code,r);return send(res,200,{code,token:p.id});}
 if(!r)throw Error('房间不存在，或服务器已重启');r.updated=Date.now();
 if(b.op==='join'){if(!['waiting','done'].includes(r.phase))throw Error('对局进行中，请等本手结束再加入');if(r.players.length>=6)throw Error('房间已满（最多 6 人）');const p=player(String(b.name||'朋友').trim().slice(0,12)||'朋友');r.players.push(p);return send(res,200,{code:r.code,token:p.id});}
 const p=r.players.find(p=>p.id===b.token&&!p.bot);if(!p)throw Error('身份已失效，请重新加入');p.lastSeen=Date.now();
 if(['start','bot','reset'].includes(b.op)&&p.id!==r.host)throw Error('只有房主可以操作');
 if(b.op==='start')start(r);
 else if(b.op==='bot'){if(!['waiting','done'].includes(r.phase)||r.players.length>=6)throw Error('当前不能添加陪练');r.players.push(player(`陪练 ${r.players.filter(p=>p.bot).length+1}`,true));}
...
setInterval(()=>{for(const [code,r] of rooms){if(Date.now()-r.updated>6*3600000){rooms.delete(code);continue;}if(['waiting','done'].includes(r.phase))continue;const p=r.players[r.turn];if(!p)continue;if(p.bot&&Date.now()>r.deadline-43500){const l=legal(r,p);act(r,p.id,l.call>p.stack*.4&&randomInt(4)===0?'fold':'call');}else if(Date.now()>r.deadline)act(r,p.id,legal(r,p).call?'fold':'call');}},500).unref();
