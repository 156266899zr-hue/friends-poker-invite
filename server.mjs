...
import {room,player,start,act,view,legal,botDecision,log,rebirth,cleanCardBack,cardBacks} from './engine.mjs';
import {xpForNet} from './progression.js';
...
function roomSession(user){for(const table of rooms.values()){const seated=table.players.find(p=>p.userId===user.id);if(seated){seated.name=user.nickname;seated.cardBack=cleanCardBack(user.card_back);seated.xp=user.xp||0;return {code:table.code,token:seated.token};}}return null;}
function actAndAward(r,id,type,amount){const users=new Map(r.players.filter(p=>p.userId).map(p=>[p.id,p.userId]));act(r,id,type,amount);if(r.phase!=='done'||r.xpAwardedHand===r.hand)return;r.xpAwardedHand=r.hand;for(const row of r.lastSettlement?.rows||[]){const userId=users.get(row.id),gain=xpForNet(row.net);if(!userId||!gain)continue;const xp=identity.awardXp(userId,gain),seated=r.players.find(p=>p.id===row.id);if(seated)seated.xp=xp;log(r,`${row.name} 获得 ${gain} XP`);}}
...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(['/api','/auth','/admin-api','/community-api'].includes(url.pathname)){
  ...
  if(b.op==='create'){
   ...
   const p=player(account.nickname,false,cleanDevice(b.deviceId),account.card_back);p.token=randomUUID();p.userId=account.id;p.xp=account.xp||0;
  ...
  if(b.op==='join'){
   ...
   const spectating=!['waiting','done'].includes(r.phase),p=player(account.nickname,false,cleanDevice(b.deviceId),account.card_back);p.token=randomUUID();p.userId=account.id;p.xp=account.xp||0;if(spectating)p.action='观战中，下手入座';r.players.push(p);identity.audit(account.id,'joinRoom',r.code);
   return send(res,200,{code:r.code,token:p.token,name:p.name,recovered:false,spectating});
  }
  const p=r.players.find(p=>p.token===b.token&&p.userId===account.id&&!p.bot);if(!p)fail(400,'身份已失效，请重新加入');p.lastSeen=Date.now();p.name=account.nickname;p.cardBack=cleanCardBack(account.card_back);p.xp=account.xp||p.xp||0;
  ...
  else if(b.op==='reset'){if(!['waiting','done'].includes(r.phase))fail(400,'请先完成本手牌');r.players.forEach(p=>{p.stack=2000;p.issued=2000;p.rebirthCount=0;p.rebirthAt=0;});r.phase='waiting';r.board=[];r.result=[];r.settlement=[];r.lastSettlement=null;r.players.forEach(p=>{p.cards=[];p.total=0;p.bet=0;p.action='等待开始';});}
  else if(b.op==='rebirth'){rebirth(r,p.id);identity.audit(account.id,'rebirth',r.code);}
  else if(b.op==='act')actAndAward(r,p.id,b.type,b.amount);
...
setInterval(()=>{if(limits.size>5000)limits.clear();for(const [code,r] of rooms){if(Date.now()>=r.expiresAt){rooms.delete(code);continue;}if(['waiting','done'].includes(r.phase))continue;const p=r.players[r.turn];if(!p)continue;if(p.bot&&Date.now()>r.deadline-43500){const move=botDecision(r,p);actAndAward(r,p.id,move.type,move.amount);}else if(Date.now()>r.deadline)actAndAward(r,p.id,legal(r,p).call?'fold':'call');}},500).unref();
