...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(b.op==='create'){if(!allowed(`create:${clientKey(req)}`,5,10*60*1000))throw Error('创建房间过于频繁，请稍后再试');if(rooms.size>=100)throw Error('房间已满，请稍后再试');let code;do{code=String(randomInt(100000,1000000));}while(rooms.has(code));const p=player(cleanName(b.name),false,cleanDevice(b.deviceId),b.cardBack);r=room(code,p);r.expiresAt=Date.now()+ROOM_MAX_MS;rooms.set(code,r);return send(res,200,{code,token:p.id,name:p.name,recovered:false});}
 ...
 if(b.op==='join'){const deviceId=cleanDevice(b.deviceId),existing=deviceId&&r.players.find(p=>!p.bot&&p.deviceId===deviceId);if(existing){existing.lastSeen=Date.now();existing.cardBack=cleanCardBack(b.cardBack||existing.cardBack);return send(res,200,{code:r.code,token:existing.id,name:existing.name,recovered:true});}if(!['waiting','done'].includes(r.phase))throw Error('对局进行中，请等本手结束再加入');if(r.players.length>=MAX_PLAYERS)throw Error('房间已满（最多 8 人）');const name=cleanName(b.name);if(r.players.some(p=>nameKey(p.name)===nameKey(name)))throw Error('该昵称已在本房间使用，请换一个称呼');const p=player(name,false,deviceId,b.cardBack);r.players.push(p);return send(res,200,{code:r.code,token:p.id,name:p.name,recovered:false});}
 ...
 else if(b.op==='bot'){if(!['waiting','done'].includes(r.phase)||r.players.length>=MAX_PLAYERS)throw Error('当前不能添加陪练');const botCount=r.players.filter(p=>p.bot).length;r.players.push(player(`陪练 ${botCount+1}`,true,'',cardBacks[(botCount+1)%cardBacks.length]));}
 ...
 else if(b.op==='skin')p.cardBack=cleanCardBack(b.cardBack);
